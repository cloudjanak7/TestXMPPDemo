...
#define TEXT_HEIGHT 30 //输入框高度
#define TEXT_WIDTH 220 //输入框宽带
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = [UIColor colorWithRed:224/255. green:228/255. blue:230/255. alpha:1];
...
- (UIButton *)toolBtn
{
    if (!_toolBtn) {
        ...
        [_toolBtn setTitle:@"表情" forState:UIControlStateNormal];
        [_toolBtn setTitle:@"键盘" forState:UIControlStateSelected];
        [_toolBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [_toolBtn setFrame:CGRectMake(270 - 24 - 10, 10, 40, 24)];
        [_toolBtn addTarget:self action:@selector(swapEmjAndKeyBoard:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)keyboardWillShow:(NSNotification*)notification{
    ...
    [UIView animateWithDuration:animationDuration animations:^{
        ...
        aFrame.origin.y = keyboardRect.origin.y - aFrame.size.height;
        self.frame = aFrame;
...
{
    CGFloat newHeight = [[self textView] sizeThatFits:CGSizeMake(textView.frame.size.width,CGFLOAT_MAX)].height - 5;
...
//    NSLog(@"newHeight %f",newHeight);
    ...
    input_Frame.origin.y = current_KeyBoard_Y - input_Frame.size.height;
//    self.frame = aFrame;
